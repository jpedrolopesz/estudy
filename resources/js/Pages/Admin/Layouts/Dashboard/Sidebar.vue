...
<template>
  <div class="bg-white justify-items-center fixed top-0 left-0 h-full w-14 border-r border-gray-300 duration-500 z-50 sidebar">
    <div class="logo_class  ">
      ...
      <button type="button">
        <svg id="menu" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
    ...
    <ul class="space-y-2">
      <h3 class="links_name px-6 pb-3 text-xs font-semibold leading-4 tracking-wider text-gray-500 uppercase md:block">Main</h3>

      <li v-for="item in menuItems" :key="item.id">
        <NavLink :href="item.route" :active="route().current(item.route)">
          <div class="flex items-center p-1">
            <svg class="w-6 h-6 mr-1" :xmlns="item.svg.xmlns" :fill="item.svg.fill" :viewBox="item.svg.viewBox" :stroke-width="item.svg.strokeWidth" :stroke="item.svg.stroke">
              <path :stroke-linecap="item.svg.path.strokeLinecap" :stroke-linejoin="item.svg.path.strokeLinejoin" v-for="path in item.svg.path.d" :d="path" />
            </svg>
            <span class="ml-3 links_name">{{ item.name }}</span>
          </div>
        </NavLink>
    ...
    <ul class=" space-y-2">
      ...
      <!-- Users
        <li>
            <NavLink :href="route('users.index')" :active="route().current('users.index')">
                <i class="nav__icon bx bx bx-user transition duration-150 ease-in-out group-hover:text-indigo-500 group-focus:text-gray-500"></i>
                <span class="links_name pt-2.5 font-semibold  transition duration-150 ease-in-out group-hover:text-indigo-500 group-focus:text-gray-500">
                Users</span>
            </NavLink>
            <span class="tooltip">Users</span>
        </li>
    ...
    <div class="profile_content">
      <ul class=" space-y-2">
        <h3 class="links_name px-6 pb-3 text-xs font-semibold leading-4 tracking-wider text-gray-500 uppercase md:block">
          Settings</h3>
